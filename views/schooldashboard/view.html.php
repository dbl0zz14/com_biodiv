...
class BioDivViewSchoolDashboard extends JViewLegacy
{
  ...
  public function display($tpl = null) 
  {
	...
	if ( $this->personId ) {
		...
		// Check whether first load - this is home page
		...
		if ( $this->firstLoad ) {
			...
			$this->badgeImg = $settingsObj->badge_icon;
			
			$this->badgesPage = "bes-badges";
			$this->communityPage = "bes-community";
			$this->trapperPage = "trapper-status";
			$this->adminPage = "bes-admin-dashboard";
			...
			$this->schoolStatus = Biodiv\SchoolCommunity::getSchoolStatus ( $this->schoolUser, $this->schoolId );
			$this->classStatus = Biodiv\SchoolCommunity::getClassStatus ( $this->schoolUser, $this->schoolId );
			if ( !$this->classStatus ) {
				$this->studentStatus = Biodiv\SchoolCommunity::getStudentStatus ( $this->schoolUser, $this->schoolId );
			}
			$this->allAwards = Biodiv\Award::getAllBlankAwards ();
			$this->collectedAwards = Biodiv\Award::getCollectedAwards ( $this->schoolUser );
			
			$this->featuredSpecies = Biodiv\SchoolCommunity::getFeaturedSpecies();
			
			$schoolPost = Biodiv\Post::getPosts ($this->schoolUser, null, 1, 1);
			
			$this->latestPost = null;
			if ( count($schoolPost->posts) > 0 ) {
				
				$this->latestPost = array_pop($schoolPost->posts);
			}
		}
