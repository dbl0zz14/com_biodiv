...
* @since 0.0.1
*/
class BioDivViewClassifyBirdsPerf extends JViewLegacy
{
    /**
     *
    ...
    public function display($tpl = null) 
    {
	  ...
	  if(!$this->photo_id){
		...
		$time_elapsed_secs = microtime(true) - $this->start;
		print "<p>Time to end just before nextSequence call = " . $time_elapsed_secs . '</p>';
	
		$this->sequence = nextSequence();
		
		$time_elapsed_secs = microtime(true) - $this->start;
		print "<p>Time to just after nextSequence call = " . $time_elapsed_secs . '</p>';
		...
		if ( $this->animal_ids ) {
			//error_log("classifybirds view, animal ids found: " . $this->animal_ids );
	  ...
	  $time_elapsed_secs = microtime(true) - $this->start;
	  print "<p>Time to end of nextSequence if/else = " . $time_elapsed_secs . '</p>';
	  ...
	  if ( count($this->sequence) > 0 ) {
		...
		$time_elapsed_secs = microtime(true) - $this->start;
		print "<p>Time to end of project filters = " . $time_elapsed_secs . '</p>';
		...
		$time_elapsed_secs = microtime(true) - $this->start;
		print "<p>Time to end of all species = " . $time_elapsed_secs . '</p>';
...
*/
		...
		$time_elapsed_secs = microtime(true) - $this->start;
		print "<p>Time to end of view code = " . $time_elapsed_secs . '</p>';
