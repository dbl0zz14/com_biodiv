...
* @since 0.0.1
*/
class BioDivViewBadgeKioskQuizResults extends JViewLegacy
{
    /**
     *
    ...
    public function display($tpl = null) 
    {
		...
		if ( !$this->personId ) $errorMsg = JText::_("COM_BIODIV_KIOSKQUIZRESULTS_NO_PERSON");
		if ( !$this->projectId ) $errorMsg = JText::_("COM_BIODIV_KIOSKQUIZRESULTS_NO_PROJECT");
		if ( !$this->topicId ) $errorMsg = JText::_("COM_BIODIV_KIOSKQUIZRESULTS_NO_TOPIC");
		if ( !$this->questions ) $errorMsg = JText::_("COM_BIODIV_KIOSKQUIZRESULTS_NO_QUESTIONS");
		if ( !$this->answers ) $errorMsg = JText::_("COM_BIODIV_KIOSKQUIZRESULTS_NO_ANSWERS");
		...
		if ( !$written ) {
			...
			$userTestId = writeTestResults ( $this->topicId, $qs, $as, $scorePercent );
			
			error_log ( "UserTest id = " . $userTestId );
			
			if ( $this->badge > 0 && $this->classId > 0 ) {
				
				Biodiv\Badge::writeBadgeProgress ( $this->schoolUser, $this->classId, $this->badge, $userTestId );
			}
