...
let areaCovered = {min_lat:45, max_lat:65, min_lon:-25, max_lon:15, lat_spacing:5, lon_spacing:8, high_zoom:9, min_zoom:4};
let mapCentre = [55,-5];
let initialZoom = 5;
let showSitesOnLoad;
...
let legend;
let geojsonSites;
let sitesShown = false;
let areasShown = false;

let discovermap;


function setUpMap ( areaCov, mapC, initialZ, showSitesOnL ) {
	
	areaCovered = areaCov;
	mapCentre = mapC;
	initialZoom = initialZ;
	showSitesOnLoad = showSitesOnL;
	
	discovermap = L.map('discovermap').setView(mapCentre, initialZoom);
